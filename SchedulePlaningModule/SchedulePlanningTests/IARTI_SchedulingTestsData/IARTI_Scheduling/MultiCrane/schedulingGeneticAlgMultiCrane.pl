...
calculate_schedule([T|Rest], CurrentTime, Acc, Schedule):-
    visit(T, BaseDur, From, Due, Pen),
    best_k_cranes(BaseDur, K, Dur),
    cranes_used(K, Cranes),
    StartTime is max(CurrentTime, From),
    FinInst is StartTime + Dur - 1,
    Delay is max(0, FinInst - Due),
    Penalty is Delay * Pen,
    NextTime is FinInst + 1,
    select_k_staffs(K, StartTime, FinInst, Staffs),
    calculate_schedule(Rest, NextTime,
        [ visit(T, StartTime, FinInst, Due, Delay, Penalty, Cranes, Staffs)
        | Acc ],
        Schedule).
...
schedule_to_json([], []).
schedule_to_json(
    [visit(Id, Start, Finish, Due, Delay, Penalty, Cranes, Staffs)|Rest],
    [ json([
        visit=Id,
        start=Start,
        finish=Finish,
